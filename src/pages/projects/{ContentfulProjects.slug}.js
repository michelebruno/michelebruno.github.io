...
export default function Project({
  data: {
    project, mdx, images: { nodes: images }, allContentfulProjects,
  },
}) {
  const {
    name, slug, roles, images: [thumbnail, cover], year, client, team, websiteUrl, description,
  } = project;
    ...
    <Layout className="mx-8" title={name} fixed={false}>
      <Typography.H1 title={name}>
        <ul className="row-start-1 col-span-2 col-start-9 pt-2 ">
          ...
          {websiteUrl
                && (
                <li>
                  <AnimatedLink href={websiteUrl} target="_blank" className="font-bold" icon={<Arrow />}>
                    Visit the
                    website
                  </AnimatedLink>
                </li>
      ...
      <article>
        ...
        <MDXProvider components={{
          Description, Marquee, Image, TextBox, Tag, Grid,
        }}
        >
          <MDXRenderer
