    ...
    <Layout fixed>
      ...
      <section className="bg-black text-white min-h-screen" ref={projectContainer}>
        <div className="mb-10 grid grid-cols-10 gap-x-8 gap-y-16 py-32 items-center content-center">
          <h2 className="col-span-6 col-start-3 text-8xl uppercase font-semibold">
            Selected projects
          </h2>
          <div className="col-span-10" />
          <div className="col-span-6 col-start-3 grid grid-cols-1 md:grid-cols-1 md:gap-x-8 gap-y-60 overflow-hidden">
            <div className=" projects-container -mx-16">
              {homeProjectSlugs.map((project, index) => (
                <ProjectCard
                  className="px-16 pb-32"
                  project={project}
                  position={index + 1}
                  key={project}
                />
