...
export function H1({ children, title, fullScreen }) {
  return (
    <header className={classNames('items-center flex flex-col justify-center', fullScreen && 'min-h-screen')}>
...
export function H2({
  children, padding, containBorder, className,
}) {
  return (
    <div className={classNames('grid grid-cols-12 gap-x-16 gap-y-8 text-5xl', padding && 'py-32', className)}>
...
export function TextBox({
  ...
  return (
    <div className={classNames('grid grid-cols-12 gap-x-16 gap-y-8 text-lg', padding && 'py-32', className)}>
      <div
        className={classNames(startClasses, containBorder ? spanClasses : 'col-span-9', 'border-t-2 border-current')}
      />
...
export function AnimatedLink({
  href, to, activeClassName, component, target, children, icon, className,
}) {
  const C = component;
  return (
    <C
